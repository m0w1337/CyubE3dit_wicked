...
void RendererWindow::Create(CyRender* renderer)
{
	...
	resolutionScaleSlider.SetValue(renderer->resolutionScale);
	resolutionScaleSlider.OnSlide([renderer](wiEventArgs args) {
		if (renderer->resolutionScale != args.fValue)
		{
			renderer->resolutionScale = args.fValue;
	...
	*/
	...
	MSAAComboBox.OnSelect([=](wiEventArgs args) {
		...
		case 0:
			...
			break;
		case 1:
			renderer->setMSAASampleCount(2);
			break;
		case 2:
			renderer->setMSAASampleCount(4);
			break;
		case 3:
			renderer->setMSAASampleCount(8);
			break;
		default:
			break;
		}
