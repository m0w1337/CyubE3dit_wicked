...
struct VertexSurface
{
	...
	inline void create(in ShaderMaterial material, in VertexInput input)
	{
		...
		wposition = mul(WORLD, position);
		if (material.IsUsingWind())
		{
			const float windweight = ((normal_wind >> 24) & 0xFF) / 255.0;
			const float waveoffset = dot(position.xyz, g_xFrame_WindDirection) * g_xFrame_WindWaveSize + (wposition.x + wposition.z) / 100;
			const float waveoffsetPrev = dot(positionPrev.xyz, g_xFrame_WindDirection) * g_xFrame_WindWaveSize +(wposition.x + wposition.z) / 100;
			const float3 wavedir	   = g_xFrame_WindDirection * windweight * g_xFrame_WindRandomness/2;
			const float3 wavedir2	   = normal * windweight * g_xFrame_WindRandomness / 4;
			...
			if (windweight > 0.5) {
				const float3 wind	  = noise1 * wavedir / 3 + wavedir * g_xFrame_WindSpeed * 0.3 - wavedir2 * g_xFrame_WindSpeed * 0.1 - noise1 * wavedir2 / 3;
				const float3 windPrev = noise1Prev * wavedir / 3 + wavedir * g_xFrame_WindSpeed * 0.3 - wavedir2 * g_xFrame_WindSpeed * 0.1 - noise1Prev * wavedir2 / 3;
				...
				const float3 wind	  = g_xFrame_WindDirection * g_xFrame_WindSpeed * position.y / 8 + noise((wposition.x ^ wposition.z) / 100 + g_xFrame_Time * g_xFrame_WindSpeed / 30) * g_xFrame_WindDirection * g_xFrame_WindSpeed * position.y / 40 + noise1 * wavedir * g_xFrame_WindSpeed * position.y / 80 - noise1 * wavedir2 * g_xFrame_WindSpeed * position.y / 80;
				const float3 windPrev = g_xFrame_WindDirection * g_xFrame_WindSpeed * positionPrev.y / 8 + noise((wposition.x ^ wposition.z) / 100 + g_xFrame_TimePrev * g_xFrame_WindSpeed / 30) * g_xFrame_WindDirection * g_xFrame_WindSpeed * positionPrev.y / 40 + noise1Prev * wavedir * g_xFrame_WindSpeed * positionPrev.y / 80 - noise1Prev * wavedir2 * g_xFrame_WindSpeed * positionPrev.y / 80;
				position.xyz += noise(g_xFrame_Time * g_xFrame_WindSpeed / 3 + waveoffset) * wavedir * g_xFrame_WindSpeed / 4;
				positionPrev.xyz += noise(g_xFrame_TimePrev * g_xFrame_WindSpeed / 3 + waveoffsetPrev) * wavedir * g_xFrame_WindSpeed / 4;
				position			  = mul(WORLD, position);
#ifdef OBJECTSHADER_INPUT_PRE
				positionPrev = mul(input.GetInstanceMatrixPrev(), positionPrev);
#else
...
inline void NormalMapping(in float4 uvsets, inout float3 N, in float3x3 TBN, out float3 bumpColor)
{
	...
	if (g_xMaterial.normalMapStrength > 0 && g_xMaterial.uvset_normalMap >= 0)
	{
		...
		normalMap.b				  = normalMap.b == 0 ? 1 : normalMap.b;
